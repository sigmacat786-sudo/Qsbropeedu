// ── State ───────────────────────────────────────────────────────────────
let questions = [];          // current draft questions, sorted by id
...
const confirmFinalizeModal = document.getElementById("confirmFinalizeModal");
const confirmFinalizeYes = document.getElementById("confirmFinalizeYes");
const confirmFinalizeNo = document.getElementById("confirmFinalizeNo");
...
async function loadDraft() {
  ...
  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}`);
...
saveIndexBtn.addEventListener("click", async () => {
  ...
  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question/${currentEditQid}/reindex`, {
    method: "POST",
...
async function moveQuestion(qid, direction) {
  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question/${qid}/move`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function triggerImageReplace(qid) {
  ...
  input.addEventListener("change", async () => {
    ...
    const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question/${pendingImageQid}/image`, {
      method: "POST",
      body: fd,
    });
    const data = await res.json();
...
saveNoteBtn.addEventListener("click", async () => {
  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question/${currentEditQid}/options-note`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
document.querySelectorAll(".answer-pick:not(.add-q-answer-pick)").forEach((btn) => {
  btn.addEventListener("click", async () => {
    ...
    const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question/${currentEditQid}/answer`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
confirmDeleteYes.addEventListener("click", async () => {
  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question/${pendingDeleteQid}`, {
    method: "DELETE",
  });
  const data = await res.json();
...
saveAddQBtn.addEventListener("click", async () => {
  ...
  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/question`, {
    method: "POST",
    body: fd,
  });
  const data = await res.json();
...
finalizeBtn.addEventListener("click", () => {
  if (!questions.length) {
    showToast("❌ Pehle kam se kam ek question add karo.");
    return;
  }
  confirmFinalizeModal.classList.remove("hidden");
});
confirmFinalizeNo.addEventListener("click", () => confirmFinalizeModal.classList.add("hidden"));
confirmFinalizeYes.addEventListener("click", async () => {
  confirmFinalizeModal.classList.add("hidden");
  finalizeBtn.disabled = true;
  finalizeBtn.textContent = "⏳ Generating...";

  const res = await fetch(`/api/draft/${encodeURIComponent(DRAFT_ID)}/finalize`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}),
  });
  ...
  if (!data.ok) {
    ...
    finalizeBtn.textContent = "Submit & Generate Quiz 🚀";
    return;
  }
  showToast("Quiz link generated ✅");
  setTimeout(() => {
